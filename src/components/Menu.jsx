...
function MenuItemCard({ item, index }) {
  ...
  if (item.img) {
    return (
      <motion.div
        ...
        style={{
          background: "#fff",
          border: open ? "1.5px solid rgba(201,148,10,0.45)" : "1px solid rgba(180,120,0,0.14)",
          boxShadow: open ? "0 12px 40px rgba(180,120,0,0.15)" : "0 2px 10px rgba(180,120,0,0.06)",
      ...
      >
        <div className="relative h-44 overflow-hidden">
          <img
            ...
            style={{ transform: open ? "scale(1.07)" : "scale(1)" }}
          />
          <div className="absolute inset-0" style={{ background: "linear-gradient(to top, rgba(30,12,0,0.72) 0%, transparent 55%)" }} />
          {item.badge && (
            <span
              className="absolute top-2.5 left-2.5 text-xs font-black px-2.5 py-1 rounded"
              style={{ background: item.badgeColor, color: "#fff8e8", letterSpacing: "0.05em" }}
            >
              {item.badge}
            </span>
          )}
          <span
            className="absolute top-2.5 right-2.5 font-black text-sm px-2.5 py-1 rounded"
          ...
          <div className="absolute bottom-0 left-0 right-0 px-4 pb-3 flex items-end justify-between">
            <div>
              ...
              {item.priceNote && <p className="text-[11px] text-white/60 mt-0.5">{item.priceNote}</p>}
            </div>
            <ChevronDown
              size={14}
              style={{ color: "rgba(255,248,232,0.6)", transform: open ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.3s", flexShrink: 0 }}
            />
          </div>
        </div>
        <AnimatePresence>
  ...
  return (
    <motion.div
      ...
      style={{
        background: open ? "#fffbf0" : "#fff",
        border: open ? "1.5px solid rgba(201,148,10,0.35)" : "1px solid rgba(180,120,0,0.13)",
        boxShadow: "0 1px 8px rgba(180,120,0,0.05)",
      }}
    >
      <div className="flex-1">
        <div className="flex flex-wrap items-center gap-2 mb-1">
          ...
          {item.badge && (
            <span className="text-[10px] font-black px-2 py-0.5 rounded" style={{ background: item.badgeColor, color: "#fff8e8" }}>
              {item.badge}
            </span>
          )}
        </div>
        <p className={`text-sm leading-relaxed ${open ? "" : "line-clamp-1"}`} style={{ color: "rgba(61,34,0,0.55)" }}>{item.desc}</p>
        {item.priceNote && open && <p className="text-xs mt-1" style={{ color: "rgba(61,34,0,0.4)" }}>{item.priceNote}</p>}
      </div>
    ...
    >
      <div className="max-w-5xl mx-auto">
        ...
        >
          ...
          <div
            className="inline-block px-5 py-3 rounded-xl text-sm max-w-xl mx-auto"
