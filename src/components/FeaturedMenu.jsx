...
export default function FeaturedMenu() {
  ...
  const { items, loading } = useMenuItems({ featuredOnly: true });
            ...
            {items.map((item, i) => (
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  ...
                  {item.badge && (
                    <span className="absolute top-3 left-3 text-xs font-black px-2.5 py-1 rounded-full" style={{ background: item.badge_color || "#c9940a", color: "#fff", letterSpacing: "0.05em" }}>
                      {item.badge}
                    </span>
                  )}
                  <span className="absolute top-3 right-3 font-black text-sm px-2.5 py-1 rounded-full" style={{ background: "#1a0800", color: "#e8b800" }}>
                ...
                <div className="px-4 py-4" style={{ borderTop: "1.5px solid #e8e0d0" }}>
                  <p className="text-sm leading-relaxed mb-4" style={{ color: "rgba(61,34,0,0.65)", lineHeight: "1.6", minHeight: "40px" }}>{item.description}</p>
                  <a
