...
export default function ContactForm() {
    ...
    return (
      <div className="rounded-2xl p-8 text-center" style={{ background: "#fff", border: "1.5px solid rgba(180,120,0,0.18)" }}>
        ...
        <p className="text-sm" style={{ color: "rgba(61,34,0,0.6)" }}>
          We got it and will get back to you soon. Check your inbox for a confirmation email.
        </p>
        <button
          onClick={() => setStatus("idle")}
          className="mt-5 px-6 py-2.5 rounded-full font-bold text-sm"
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl p-6 flex flex-col gap-4" style={{ background: "#fff", border: "1.5px solid rgba(180,120,0,0.18)" }}>
      ...
      {status === "error" && (
        <p className="text-sm text-red-500">Something went wrong. Please try again or call us at 330-510-8875.</p>
      )}

      <button
        type="submit"
        disabled={status === "sending"}
        className="w-full py-3.5 rounded-xl font-black text-sm tracking-widest uppercase transition-opacity hover:opacity-85"
        style={{ background: "#c9940a", color: "#fff8e8", opacity: status === "sending" ? 0.7 : 1 }}
      >
