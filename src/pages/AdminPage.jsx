...
const TABS = ['Truck Status','Menu Items','Events','Reports','Broadcast','Catering'];
        ...
        {['all', ...STATUS_ORDER].map(s => {
          ...
          return (
            <button
              ...
              style={{
                background: isActive ? (meta?.bg ?? '#1a0800') : '#fff',
                color: isActive ? (meta?.color ?? '#e8b800') : 'rgba(61,34,0,0.5)',
                border: `1.5px solid ${isActive ? (meta?.dot ?? '#1a0800') : '#e8e0d0'}`,
                cursor: 'pointer',
              }}
            >
        ...
        {filtered.map(inq => {
          ...
          return (
            <div
              ...
              style={{background:'#fff', border:'1.5px solid rgba(180,120,0,0.15)', boxShadow:'0 1px 8px rgba(180,120,0,0.05)'}}
            >
              {/* Row */}
              <button
                type="button"
                ...
                <div className="px-4 pb-5 flex flex-col gap-4" style={{borderTop:'1px solid rgba(180,120,0,0.08)'}}>
                  ...
                  <div className="grid grid-cols-2 gap-3 pt-4">
                    <div>
                      ...
                      <a href={`mailto:${inq.email}`} className="text-sm font-bold" style={{color:'#1a0800',textDecoration:'none'}}>{inq.email}</a>
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest mb-1" style={{color:'#c9940a'}}>Phone</p>
                      <a href={`tel:${inq.phone}`} className="text-sm font-bold" style={{color:'#1a0800',textDecoration:'none'}}>{inq.phone || '—'}</a>
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest mb-1" style={{color:'#c9940a'}}>Guests</p>
                      <p className="text-sm font-bold" style={{color:'#1a0800'}}>{inq.guest_count || '—'}</p>
                    </div>
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest mb-1" style={{color:'#c9940a'}}>Received</p>
                      <p className="text-sm font-bold" style={{color:'#1a0800'}}>{fmt(inq.created_at)}</p>
                    </div>
                  </div>
                  ...
                  {inq.message && (
                    <div>
                      <p className="text-xs font-black uppercase tracking-widest mb-1" style={{color:'#c9940a'}}>Message</p>
                      <p className="text-sm leading-relaxed" style={{color:'rgba(61,34,0,0.7)'}}>{inq.message}</p>
                    </div>
                  )}
                      ...
                      {STATUS_ORDER.map(s => {
                        ...
                        return (
                          <button
                            ...
                            disabled={active}
                            className="px-3 py-1.5 rounded-full text-xs font-bold"
                            style={{background: active ? m.bg : '#fff', color: active ? m.color : 'rgba(61,34,0,0.5)', border:`1.5px solid ${active ? m.dot : '#e8e0d0'}`, cursor: active ? 'default' : 'pointer', opacity: active ? 1 : 0.85}}
                          >
                            {m.label}
                          </button>
                        );
                  ...
                  <div className="flex gap-2 pt-1">
                    <a
                      href={`mailto:${inq.email}?subject=Your Cheezies Catering Inquiry&body=Hi ${inq.name},%0A%0AThank you for your interest in Cheezies Gourmet catering!`}
      ...
      <div className="flex border-b overflow-x-auto" style={{background:'#fff',borderColor:'#e8e0d0'}}>
        {TABS.map((t, i) => (
          <button key={t} onClick={() => setTab(i)}
            ...
            style={{background:'none',border:'none',borderBottom:`2.5px solid ${tab===i?'#c9940a':'transparent'}`,color:tab===i?'#c9940a':'rgba(61,34,0,0.45)',cursor:'pointer',whiteSpace:'nowrap'}}>{t}</button>
        ))}
      </div>
