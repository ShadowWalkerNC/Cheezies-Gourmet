...
const label = (text) => <label className="text-xs font-black uppercase tracking-widest mb-1.5 block" style={{color:'#c9940a'}}>{text}</label>;
...
function TruckTab() {
  ...
  const toggleDay = (day) => setForm(f => ({...f, open_days: f.open_days?.includes(day) ? f.open_days.filter(d=>d!==day) : [...(f.open_days||[]), day]}));

  const handleSave = async (e) => {
    e.preventDefault(); setSaving(true);
    const ok = await saveTruckData(form);
    setSaving(false); if (ok) { setSaved(true); setTimeout(() => setSaved(false), 2500); }
  };
  ...
  return (
    <form onSubmit={handleSave} className="flex flex-col gap-5">
        ...
        <div className="flex gap-2">
          {['open','closed','en_route'].map(s => (
            <button type="button" key={s} onClick={() => set('status', s)}
              ...
              style={{background: form.status===s?'#c9940a':'#fff', color:form.status===s?'#fff':'#7a4f00', border:`1.5px solid ${form.status===s?'#c9940a':'#e8e0d0'}`}}>{s.replace('_',' ')}</button>
          ))}
        </div>
      </div>
      <div>{label("Today's Address")}<input value={form.address||''} onChange={e=>set('address',e.target.value)} placeholder="e.g. 123 Main St, Akron" style={inp} /></div>
      <div className="grid grid-cols-2 gap-3">
        <div>{label('Latitude')}<input value={form.latitude||''} onChange={e=>set('latitude',e.target.value)} placeholder="41.0814" style={inp} /></div>
        <div>{label('Longitude')}<input value={form.longitude||''} onChange={e=>set('longitude',e.target.value)} placeholder="-81.5190" style={inp} /></div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>{label('Opens')}<input value={form.hours_open||''} onChange={e=>set('hours_open',e.target.value)} placeholder="11:00 AM" style={inp} /></div>
        <div>{label('Closes')}<input value={form.hours_close||''} onChange={e=>set('hours_close',e.target.value)} placeholder="6:00 PM" style={inp} /></div>
      </div>
      <div>{label('Open Days')}
        <div className="flex flex-wrap gap-2">
          {DAYS.map(day => (
            <button type="button" key={day} onClick={() => toggleDay(day)}
              ...
              style={{background:form.open_days?.includes(day)?'#c9940a':'#fff', color:form.open_days?.includes(day)?'#fff':'#7a4f00', border:`1.5px solid ${form.open_days?.includes(day)?'#c9940a':'#e8e0d0'}`}}>{day.slice(0,3)}</button>
          ))}
        </div>
      </div>
      <div>{label("Today's Note (optional)")}<textarea rows={2} value={form.note||''} onChange={e=>set('note',e.target.value)} placeholder="e.g. Cash only today!" style={{...inp, resize:'none'}} /></div>
...
function MenuTab() {
  ...
  const BLANK = { name:'', section:'Signature Creations', description:'', price:0, price_text:'', price_note:'', badge:'', badge_color:'#c9940a', image_url:'', is_featured:false, is_active:true, sort_order:items.length+1 };
  ...
  if (editing) return (
    <form onSubmit={saveItem} className="flex flex-col gap-4">
      ...
      <div>{label('Name')}<input required value={editing.name} onChange={e=>setEditing(f=>({...f,name:e.target.value}))} style={inp} /></div>
      <div>{label('Section')}
        <select value={editing.section} onChange={e=>setEditing(f=>({...f,section:e.target.value}))} style={inp}>
          {SECTIONS.map(s => <option key={s} value={s}>{s}</option>)}
        </select>
      </div>
      <div>{label('Description')}<textarea rows={3} value={editing.description} onChange={e=>setEditing(f=>({...f,description:e.target.value}))} style={{...inp,resize:'none'}} /></div>
      <div className="grid grid-cols-2 gap-3">
        <div>{label('Price Display (e.g. $13)')}<input value={editing.price_text} onChange={e=>setEditing(f=>({...f,price_text:e.target.value}))} placeholder="$13" style={inp} /></div>
        <div>{label('Price Note')}<input value={editing.price_note||''} onChange={e=>setEditing(f=>({...f,price_note:e.target.value}))} placeholder="Single / Double" style={inp} /></div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div>{label('Badge Text')}<input value={editing.badge||''} onChange={e=>setEditing(f=>({...f,badge:e.target.value}))} placeholder="Best Seller" style={inp} /></div>
        <div>{label('Badge Color')}<input type="color" value={editing.badge_color||'#c9940a'} onChange={e=>setEditing(f=>({...f,badge_color:e.target.value}))} style={{...inp,height:42,padding:4}} /></div>
      </div>
      <div>{label('Image URL')}<input value={editing.image_url||''} onChange={e=>setEditing(f=>({...f,image_url:e.target.value}))} placeholder="https://..." style={inp} /></div>
      ...
      <div>{label('Sort Order')}<input type="number" value={editing.sort_order} onChange={e=>setEditing(f=>({...f,sort_order:parseInt(e.target.value)}))} style={inp} /></div>
        ...
        {items.map(item => (
          <div key={item.id} className="flex items-center justify-between gap-3 p-3 rounded-2xl" style={{background:'#fff',border:`1.5px solid ${item.is_active?'rgba(180,120,0,0.15)':'rgba(180,120,0,0.07)'}`,opacity:item.is_active?1:0.5}}>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs" style={{color:'rgba(61,34,0,0.45)'}}>{item.section} · {item.price_text || `$${item.price}`}{item.is_featured?' · ⭐ Featured':''}</p>
            </div>
...
function EventsTab() {
  ...
  const BLANK = { title:'', event_date:'', event_time:'', time_start:'', time_end:'', location_name:'', address:'', description:'', image_url:'', facebook_event_url:'', is_featured:false, is_active:true };
  ...
  const saveEvent = async (e) => {
    e.preventDefault(); setSaving(true);
    if (editing.id) { await supabase.from('events').update(editing).eq('id', editing.id); }
    else { await supabase.from('events').insert(editing); }
    setSaving(false); setEditing(null); load();
  };
  ...
  if (editing) return (
    <form onSubmit={saveEvent} className="flex flex-col gap-4">
      <button type="button" onClick={() => setEditing(null)} className="text-sm font-bold text-left" style={{color:'#c9940a',background:'none',border:'none',cursor:'pointer'}}>← Back to list</button>
      <div>{label('Title')}<input required value={editing.title} onChange={e=>setEditing(f=>({...f,title:e.target.value}))} style={inp} /></div>
      <div>{label('Date')}<input type="date" value={editing.event_date||''} onChange={e=>setEditing(f=>({...f,event_date:e.target.value}))} style={inp} /></div>
      <div className="grid grid-cols-2 gap-3">
        <div>{label('Start Time')}<input value={editing.time_start||''} onChange={e=>setEditing(f=>({...f,time_start:e.target.value}))} placeholder="10:00 AM" style={inp} /></div>
        <div>{label('End Time')}<input value={editing.time_end||''} onChange={e=>setEditing(f=>({...f,time_end:e.target.value}))} placeholder="4:00 PM" style={inp} /></div>
      </div>
      <div>{label('Location Name')}<input value={editing.location_name||''} onChange={e=>setEditing(f=>({...f,location_name:e.target.value}))} placeholder="Akron Civic" style={inp} /></div>
      <div>{label('Address')}<input value={editing.address||''} onChange={e=>setEditing(f=>({...f,address:e.target.value}))} placeholder="182 S Main St, Akron" style={inp} /></div>
      <div>{label('Description')}<textarea rows={3} value={editing.description||''} onChange={e=>setEditing(f=>({...f,description:e.target.value}))} style={{...inp,resize:'none'}} /></div>
      <div>{label('Image URL')}<input value={editing.image_url||''} onChange={e=>setEditing(f=>({...f,image_url:e.target.value}))} placeholder="https://..." style={inp} /></div>
      <div>{label('Facebook Event URL')}<input value={editing.facebook_event_url||''} onChange={e=>setEditing(f=>({...f,facebook_event_url:e.target.value}))} placeholder="https://facebook.com/events/..." style={inp} /></div>
      <div className="flex gap-4">
          ...
          {events.map(ev => (
            <div key={ev.id} className="flex items-center justify-between gap-3 p-3 rounded-2xl" style={{background:'#fff',border:'1.5px solid rgba(180,120,0,0.15)'}}>
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs" style={{color:'rgba(61,34,0,0.45)'}}>{ev.event_date} · {ev.location_name||ev.location||'No location'}</p>
              </div>
              <div className="flex gap-2 shrink-0">
